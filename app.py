...
def get_time_index(t_str):
    return TIME_OPTIONS.index(t_str) if t_str in TIME_OPTIONS else 0
...
def render_privacy_policy():
    with st.expander("📜 개인정보 처리방침 (수업용 웹 앱)", expanded=False):
        ...
        **1. 개인정보의 수집 및 이용 목적**
        - 교과 수업 운영, 학생 수행평가 과제물 제출 및 취합, 피드백 제공 및 학생부 기재 증빙 자료 활용
        ...
        **4. 개인정보의 안전성 확보 조치**
        - 비밀번호 마스킹(비노출) 처리 및 교사/학생 간 접근 권한 분리 통제
        - 데이터 유실 및 훼손 방지를 위한 자동 스냅샷 백업 시스템 운영

        **5. 정보주체의 권리 행사 (열람/정정/삭제 요구 절차)**
        - 학생은 언제든지 본인의 개인정보 및 제출된 과제 내역에 대해 열람, 정정, 삭제를 요청할 수 있습니다.
        - 수정 및 삭제는 담당 교사에게 요청하거나 관리자 화면의 회원 관리 기능을 통해 즉시 처리됩니다.

        **6. 만 14세 미만 아동의 개인정보 보호**
        - 본 프로그램은 고등학교 교육과정 재학생(만 14세 이상) 전용으로 운영됩니다.

        **7. 개인정보 보호책임자**
        - 책임자: 교사 김명남 / 소속: 신선여자고등학교
    ...
    for slot in slots:
        if slot['day'] != "선택안함":
            ...
            if slot['day'] == current_day:
                try:
                    if datetime.datetime.strptime(slot["start"], "%H:%M").time() <= current_time <= datetime.datetime.strptime(slot["end"], "%H:%M").time():
                        is_time_match = True
                except:
                    continue
...
def create_auto_backup(reason="자동 스냅샷"):
    with db_lock:
        try:
            ...
            with open(os.path.join(BACKUP_DIR, f"backup_{now_str}.json"), "w", encoding="utf-8") as f:
    ...
    with db_lock:
        ...
        for key in ["materials", "notices", "custom_blocks", "dynamic_links"]:
            if key not in current_config:
                current_config[key] = []
                needs_update = True
        if "activity_visibility" not in current_config:
            current_config["activity_visibility"] = {}
            needs_update = True
        ...
        if ACT_2_3 not in current_config["subject_activities"].get("2학년 도시의 미래 탐구", []):
            current_config["subject_activities"].setdefault("2학년 도시의 미래 탐구", []).append(ACT_2_3)
            needs_update = True
    ...
    if act_name in [ACT_2_1, ACT_2_2]:
        ...
        for k, acts in learning_data.items():
            if k.startswith(f"{u_subj}_{u_class}_"):
                if u_id_str in [str(acts.get(act_name, {}).get(f"m{i}_id", "")).strip() for i in range(1, 5)]:
                    return k, acts.get(act_name, {})
    return user_key, learning_data.get(user_key, {}).get(act_name, {})
...
# 🌟 [오류 수정 반영] NameError 방지를 위한 config 자동 로드
def generate_html_content(act_name, ans, config=None):
    ...
    if act_name == ACT_3_1:
        ...
        if b64_before:
            ...
            else: html += "<p>(웹 포트폴리오 상에서는 미리보기를 제공하지 않습니다. 원본을 다운로드해주세요.)</p>"
        ...
        html += "<h3>Step 1. 우리 지역 정체성 자원 발굴 및 팩트 체크</h3><table><tr><th>구분</th><th>내가 찾은 정체성 키워드 혹은 문장</th><th>근거가 되는 사실·통계·사건</th><th>출처(기관명/자료명/연도)</th></tr>"
        ...
        html += f"</table><p><b>▶ 내가 최종 선택한 핵심 키워드 혹은 문장:</b> {ans.get('step1_keyword','')}</p><p><b>▶ 내 작품이 전할 단 하나의 메시지:</b> {ans.get('step1_message','')}</p>"
        html += "<h3>Step 2. 캔버스 선정</h3><table><tr><th>후보(건물명)</th><th>벽면 조건</th><th>관람 조건</th><th>접근성</th><th>예상 제약</th><th>지역 정체성 연관성</th><th>적합도</th></tr>"
        ...
        html += f"</table><p><b>▶ 최종 선정 건물:</b> {ans.get('step2_final_building','')}</p><p><b>▶ 선정 이유:</b> {ans.get('step2_reason','')}</p>"
        html += "<h3>Step 3. 주어진 조건 진단 및 대응 설계</h3><table><tr><th>조건 영역</th><th>현장의 실제 조건</th><th>작품에 미치는 영향</th><th>나의 대응 방안</th></tr>"
        ...
        html += "</table><h3>Step 4. 작품 스토리보드 4컷</h3>"
        for i in range(1, 5):
            html += f"<h4>컷 {i}</h4><p><b>장면 설명:</b> {ans.get(f'cut_{i}_desc', '')}</p>"
            b64_file = ans.get(f"cut_{i}_file_data", "")
            if b64_file:
                html += f"<p>첨부 파일: {ans.get(f'cut_{i}_file_name', '')}</p>"
                if str(ans.get(f'cut_{i}_file_name', '')).lower().endswith(('.png', '.jpg', '.jpeg', '.gif')):
                    html += f"<img src='data:image/png;base64,{b64_file}' style='max-width:100%; border:1px solid #ccc;'/>"
        html += f"<h3>Step 5. 작품 설명 카드 작성 및 갤러리 워크</h3><p><b>작품 제목:</b> {ans.get('step5_title','')}</p><p><b>전시 장소:</b> {ans.get('step5_place','')}</p><p><b>작품 개요:</b> {ans.get('step5_summary','')}</p><p><b>지역 정체성 반영:</b> {ans.get('step5_identity','')}</p><p><b>현장 조건 반영:</b> {ans.get('step5_condition','')}</p><p><b>우리 지역에 남길 변화:</b> {ans.get('step5_change','')}</p>"
        html += "<h3>Step 6. 제출 전 자기 점검 및 활용 기록</h3><h4>점검 항목 체크리스트</h4><table><tr><th>No</th><th>점검 항목</th><th>확인 여부</th></tr>"
        for row in ans.get("step6_chk_df", []): html += f"<tr><td>{row.get('No','')}</td><td>{row.get('점검 항목','')}</td><td>{'✅' if row.get('확인') else '❌'}</td></tr>"
        html += "</table><h4>생성형 AI 활용 기록</h4><table><tr><th>사용한 도구명</th><th>입력한 프롬프트</th><th>AI 결과물을 내가 수정·판단한 내용</th></tr>"
        ...
        html += f"</table><p><b>▶ 활동 성찰:</b> {ans.get('step6_reflection','')}</p>"
    else:
            ...
            if st.form_submit_button("가입 신청", type="primary", use_container_width=True):
                if reg_subject and reg_class and reg_id and reg_name and reg_pw:
                    if not agree_privacy:
                        st.error("❌ 개인정보 수집 및 이용에 동의해야 가입할 수 있습니다.")
                    else:
