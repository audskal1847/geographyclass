...
/* 🔵 Secondary 버튼 (백업, 복구 다운로드 등) - 파란색 복원 */
...
# 🌟 NameError 방지: 개인정보 처리방침 최상단 정의
def render_privacy_policy():
    with st.expander("📜 개인정보 처리방침 (수업용 웹 앱)", expanded=False):
        st.markdown("""
        **[신선여자고등학교 수업용 웹 앱 개인정보 처리방침]**
        **1. 수집 목적**: 교과 수업 운영, 수행평가 과제물 제출 확인 및 피드백, 학교생활기록부 기재 증빙 자료 활용
        **2. 수집 항목**: 필수 항목(과목, 반, 학번, 이름, 비밀번호, 과제물 데이터). ※ 주민등록번호 등 민감정보 일체 수집 금지
...
# --- [1] 권한 및 기한 제어 (특정 학생 예외 처리 기능 탑재) ---
...
def render_activity1_3th(user_key, u_info, current_role):
    ...
    if not disabled_flag:
        if st.button("저장하기", type="primary", key=f"save_{category}"):
            ...
            save_json(DATA_FILE, current_data); create_auto_backup(f"[{u_name}] {category} 활동 저장"); st.balloons(); st.success("🎉 화면 저장이 완료되었습니다!")
...
def render_activity2_2nd(user_key, u_info, current_role):
    ...
    default_step1_2 = [{"구분": "주거 및 생활", "필수 서비스 항목": "생필품 마트, 일상 편의시설", "충분": False, "부족 or 없음": False}, {"구분": "의료 및 돌봄", "필수 서비스 항목": "병원, 약국, 돌봄센터", "충분": False, "부족 or 없음": False}, {"구분": "노동 및 학습", "필수 서비스 항목": "청소년 무료 학습/스터디 공간", "충분": False, "부족 or 없음": False}, {"구분": "여가 및 녹지", "필수 서비스 항목": "공원, 수변 공간, 휴식 공간", "충분": False, "부족 or 없음": False}, {"구분": "교육 및 문화", "필수 서비스 항목": "도서관, 학습 공간, 문화 시설", "충분": False, "부족 or 없음": False}, {"구분": "이동 및 보행", "필수 서비스 항목": "보행자 전용 도로, 자전거 도로", "충분": False, "부족 or 없음": False}]
    ...
    default_point_table = [{"카테고리": "안전한 보행 환경", "코드": "A-1", "세부 개조 항목": "여고생 안심 귀가 스마트 로드 (CCTV 연동)", "비용": "-15pt"}, {"카테고리": "", "코드": "A-2", "세부 개조 항목": "아파트 단지 간 담장 철거 및 공공 보행로 연결", "비용": "-20pt"}, {"카테고리": "", "코드": "A-3", "세부 개조 항목": "차로 축소 및 쾌적한 보행을 위한 녹지 공간 조성", "비용": "-20pt"}, {"카테고리": "", "코드": "A-4", "세부 개조 항목": "스마트 횡단보도 및 교통약자/학생 쉼터", "비용": "-10pt"}, {"카테고리": "", "코드": "A-5", "세부 개조 항목": "야간 자율학습 후 안전 귀가를 위한 셉테드(CPTED) 조명", "비용": "-10pt"}, {"카테고리": "", "코드": "A-6", "세부 개조 항목": "", "비용": ""}, {"카테고리": "녹지 및 생태공간 구축", "코드": "B-1", "세부 개조 항목": "아파트 상가/방치 공터 → 도심 소공원 조성", "비용": "-15pt"}, {"카테고리": "", "코드": "B-2", "세부 개조 항목": "도심 바람길 숲 및 수변 산책로 조성", "비용": "-15pt"}, {"카테고리": "", "코드": "B-3", "세부 개조 항목": "에코 펫파크(반려견 전용 공원 및 산책로)", "비용": "-15pt"}, {"카테고리": "", "코드": "B-4", "세부 개조 항목": "아파트 벽면 녹화 및 옥상 정원(학생 쉼터) 조성", "비용": "-15pt"}, {"카테고리": "", "코드": "B-5", "세부 개조 항목": "", "비용": ""}, {"카테고리": "문화와 교육을 위한 공간", "코드": "C-1", "세부 개조 항목": "24시간 공공 스터디 & 커뮤니티 카페", "비용": "-15pt"}, {"카테고리": "", "코드": "C-2", "세부 개조 항목": "청소년 팝업 스튜디오 & 소공연장 (여고생 동아리 특화)", "비용": "-15pt"}, {"카테고리": "", "코드": "C-3", "세부 개조 항목": "친환경 스마트 팜", "비용": "-10pt"}, {"카테고리": "", "코드": "C-4", "세부 개조 항목": "프리미엄 복합 문화 공간 (북카페, 피트니스 존 등)", "비용": "-20pt"}, {"카테고리": "", "코드": "C-5", "세부 개조 항목": "", "비용": ""}, {"카테고리": "효율적인 교통과 모빌리티 구축", "코드": "D-1", "세부 개조 항목": "공유 자전거 및 킥보드 전용 도로", "비용": "-15pt"}, {"카테고리": "", "코드": "D-2", "세부 개조 항목": "스마트 버스 쉘터(공기 청정, 온열 의자 구축)", "비용": "-10pt"}, {"카테고리": "", "코드": "D-3", "세부 개조 항목": "등하교 혼잡 방지용 아파트 단지 앞 스마트 승하차 존", "비용": "-15pt"}, {"카테고리": "", "코드": "D-4", "세부 개조 항목": "", "비용": ""}]
...
def render_activity3_2nd(user_key, u_info, current_role):
    ...
    if not disabled_flag and st.button("저장하기", type="primary", key=f"save_{category}"):
        ...
        create_auto_backup(f"[{u_name}] {category} 활동 저장"); st.balloons(); st.success("🎉 화면 저장이 완료되었습니다!")
...
def render_custom_activity(user_key, u_info, current_role, act_name, config):
    ...
    if not disabled_flag and st.button("저장하기", type="primary", key=f"save_{act_name}"):
        ...
        save_json(DATA_FILE, current_data); ans = new_ans
        create_auto_backup(f"[{u_name}] {act_name} 저장"); st.balloons(); st.success("🎉 화면 저장이 완료되었습니다!")
    ...
    if st.session_state.current_page != "main":
        ...
        if current_role == "학생":
                ...
                with col_add:
                    ...
                    if st.button("추가하기", type="primary"):
                        if new_act_name and new_act_name not in acts_list:
                            ...
                            save_json(CONFIG_FILE, fresh_config); create_auto_backup(f"[{edit_subj}] 새 수행평가 추가: {new_act_name}"); st.success("추가 완료"); st.rerun()
                with col_del:
                    if acts_list:
                        ...
                        if del_act_name != "선택" and st.button("영구 삭제", type="primary"):
                            ...
                            save_json(CONFIG_FILE, fresh_config); create_auto_backup(f"[{edit_subj}] 수행평가 삭제: {del_act_name}"); st.success("삭제 완료"); st.rerun()
                ...
                if pending_users:
                    ...
                    with col_p1:
                        ...
                        if approve_class_sel != "선택" and st.button(f"✅ [{approve_class_sel}] 일괄 승인", type="primary", use_container_width=True):
                            ...
                            save_json(USERS_FILE, fresh_users); create_auto_backup(f"[{approve_class_sel}] 반별 승인 ({count}명)"); st.success("승인 완료"); st.rerun()
                    with col_p2:
                    ...
                    with col_p3:
                        ...
                        if st.button("✅ 모든 대기 학생 일괄 승인", type="primary", use_container_width=True):
                            ...
                            save_json(USERS_FILE, fresh_users); create_auto_backup("전체 대기 학생 일괄 승인"); st.success("승인 완료"); st.rerun()
                else: 
                    st.info("대기 중인 학생이 없습니다.")
                    
                st.markdown("---")
                # 🌟 [요청 기능] 이중 가입 학생 계정 삭제 기능 추가 및 복원
            ...
            with menu_tabs[4]:
                ...
                # 🌟 [스마트 파일 파서] 인코딩(BOM) 에러 원천 차단 및 스냅샷 자동 분해 기능
                ...
                with col_bk2:
                    ...
                    if st.button("학생 학습 데이터 복구 실행", use_container_width=True): # 🌟 파란색 버튼 복원 (type 제거)
                        if up_data:
                            ...
                            else: st.error("❌ 올바른 json 파일이 아니거나 인코딩이 손상되었습니다.")
