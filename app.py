...
def load_json(file_path, default_value):
    with db_lock:
        ...
        for _ in range(5):
            ...
            except json.JSONDecodeError: import time; time.sleep(0.1)
        return default_value
    ...
    with db_lock:
        try:
            ...
            backup_bundle = {
                ...
                "reason": reason, "users": load_json(USERS_FILE, {}),
                "learning_data": load_json(DATA_FILE, {}), "config": load_json(CONFIG_FILE, {})
            }
...
def init_system():
    with db_lock:
        ...
        for k in keys_to_delete: del users[k]; users_changed = True
        ...
        for key in ["materials", "notices", "custom_blocks", "dynamic_links"]:
            if key not in current_config: current_config[key] = []; needs_update = True
        ...
        if "subject_activities" not in current_config:
            current_config["subject_activities"] = {"3학년 여행지리": [ACT_3_1, ACT_3_2, ACT_3_3], "2학년 도시의 미래 탐구": [ACT_2_1, ACT_2_2]}
            needs_update = True
        if ACT_2_3 not in current_config["subject_activities"].get("2학년 도시의 미래 탐구", []):
            current_config["subject_activities"].setdefault("2학년 도시의 미래 탐구", []).append(ACT_2_3); needs_update = True
...
def get_act_csv_rows(selected_view, ans, config=None):
    ...
    if selected_view == ACT_3_1:
        csv_data.extend([["1. 영상 제목", ans.get("a1_1", "")], ["2. 등장 국가/지역", ans.get("a1_2", "")], ["3. 선택 이유", ans.get("a1_3", "")], ["1. 첫 느낌", ans.get("a2_1", "")], ["2. 인상적 장소", ans.get("a2_2_1", "")], ["이유", ans.get("a2_2_2", "")], ["3. 누구에게 추천", ans.get("a2_3_1", "")], ["추천 이유", ans.get("a2_3_2", "")], ["4. 나만의 감상평", ans.get("a2_4", "")], ["1) 영상 제목", ans.get("a3_1", "")], ["2) 주요 컨셉/느낌", ans.get("a3_2", "")], ["3) 누구와 함께?", ans.get("a3_3", "")], ["4) 그 이유는?", ans.get("a3_4", "")], ["5) 가장 해보고 싶은 것", ans.get("a3_5", "")], ["6) 그 이유는?", ans.get("a3_6", "")], ["7) 꼭 넣고 싶은 장소", ans.get("a3_7", "")], ["8) 그 이유는?", ans.get("a3_8", "")], ["9) 썸네일 기획", ans.get("a3_9", "")], ["10) 어울리는 BGM", ans.get("a3_10", "")], ["11) BGM 선택 이유", ans.get("a3_11", "")]])
    elif selected_view == ACT_3_2:
        csv_data.extend([["1-1 편안한 장소", ans.get("q1_1", "")], ["1-2 편안한 이유", ans.get("q1_2", "")], ["2-1 성격", ans.get("q2_1", "")], ["2-2 성격형성 장소", ans.get("q2_2", "")], ["2-3 그 이유", ans.get("q2_3", "")], ["3-1 장점", ans.get("q3_1", "")], ["3-2 장점형성 장소", ans.get("q3_2", "")], ["3-3 그 이유", ans.get("q3_3", "")], ["4-1 성장영향 장소", ans.get("q4_1", "")], ["4-2 어떤면에서", ans.get("q4_2", "")], ["5-1 나의 목표", ans.get("q5_1", "")], ["5-2 목표영향 장소", ans.get("q5_2", "")], ["6-1 소개장소", ans.get("q6_1", "")], ["6-2 그 이유", ans.get("q6_2", "")], ["7-1 비밀장소", ans.get("q7_1", "")], ["7-2 그 이유", ans.get("q7_2", "")], ["8-1 과거로 간다면", ans.get("q8_1", "")], ["8-2 그 이유", ans.get("q8_2", "")]])
    elif selected_view == ACT_3_3:
        for row in ans.get("s1_df", []): csv_data.append([row.get("대륙", ""), f"관심도: {row.get('관심도', '')} / 지식: {row.get('지식수준', '')}"])
        for row in ans.get("direct_df", []): csv_data.append([row.get("여행해 본 국가", ""), row.get("기억/인상", "")])
        csv_data.extend([["간접경험(영화/드라마)", ans.get("ind1", "")], ["간접경험(음악)", ans.get("ind2", "")], ["간접경험(음식)", ans.get("ind3", "")]])
        for row in ans.get("top5_want", []): csv_data.append([row.get("가고싶은 국가", ""), row.get("이유", "")])
        for row in ans.get("top5_notwant", []): csv_data.append([row.get("가기싫은 국가", ""), row.get("이유", "")])
        for row in ans.get("label_df", []): csv_data.append([row.get("가고싶은 국가", ""), f"라벨: {row.get('한 단어 라벨', '')} / 싫은국가: {row.get('가고 싶지 않은 국가', '')} / 라벨(부정): {row.get('한 단어 라벨(부정)', '')}"])
        for row in ans.get("prej_df", []): csv_data.append([row.get("국가명", ""), f"편견내용: {row.get('편견 내용', '')} / 형성과정: {row.get('편견 형성 과정 혹은 이유', '')}"])
        csv_data.extend([["뉴스 빈도 국가", ans.get("media1_1", "")], ["뉴스 이미지", ans.get("media1_2", "")], ["영화 빈도 국가", ans.get("media2_1", "")], ["영화 이미지", ans.get("media2_2", "")], ["학교 배운 국가", ans.get("media3_1", "")], ["학교 지식", ans.get("media3_2", "")]])
        for row in ans.get("fake_df", []): csv_data.append([row.get("국가명", ""), f"잘못알던내용: {row.get('잘못 알고 있었던 내용', '')} / 실제사실: {row.get('실제 사실', '')}"])
        for row in ans.get("discrim_df", []): csv_data.append([row.get("어떤 국가?", ""), f"어떤측면: {row.get('어떤 측면에서', '')} / 이유: {row.get('그 이유', '')}"])
        for row in ans.get("change_df", []): csv_data.append([row.get("어떤 국가?", ""), f"현재편견: {row.get('현재의 편견', '')} / 수집계획: {row.get('올바른 정보를 찾기 위한 계획', '')}"])
        for row in ans.get("ignore_df", []): csv_data.append([row.get("대륙/국가", ""), f"무관심이유: {row.get('무관심 이유', '')} / 확장방법: {row.get('관심 확장을 위한 정보 수집 방법', '')}"])
        for row in ans.get("western_df", []): csv_data.append([row.get("서구중심적 시각", ""), f"개선방법: {row.get('개선 방법', '')}"])
        csv_data.extend([["되고싶은 사람", ans.get("goal_1", "")], ["갖고싶은 세계관", ans.get("goal_2", "")]])
    elif selected_view == ACT_2_1:
        csv_data.append(["모둠원", f"{ans.get('m1_id','')} {ans.get('m1_name','')} / {ans.get('m2_id','')} {ans.get('m2_name','')} / {ans.get('m3_id','')} {ans.get('m3_name','')} / {ans.get('m4_id','')} {ans.get('m4_name','')}"])
        ...
        for row in ans.get("step2_point_df", []): csv_data.append([row.get("카테고리", ""), f"코드: {row.get('코드', '')}, 내용: {row.get('세부 개조 항목', '')}, 비용: {row.get('비용', '')}"])
        for row in ans.get("step2_df", []): csv_data.append([f"순번 {row.get('순번', '')}", f"코드: {row.get('선택 코드', '')} / 버릴공간: {row.get('버릴 공간', '')} / 포인트: {row.get('사용 포인트', '')} / 재설계: {row.get('공간 재설계 이유 및 기대효과', '')}"])
        csv_data.extend([["슬로건", ans.get("step4_1", "")], ["공간 문제", ans.get("step4_2", "")], ["버리고 채운 것", ans.get("step4_3", "")], ["일상 변화", ans.get("step4_4", "")]])
    elif selected_view == ACT_2_3:
        csv_data.extend([["학번", ans.get('ind_id', '')], ["이름", ans.get('ind_name', '')], ["희망 진로", ans.get("ind_career", "")]])
        ...
        for row in ans.get("step2_df", []): csv_data.append([row.get("건물명", ""), f"벽면: {row.get('벽면 조건', '')} / 관람: {row.get('관람 조건', '')} / 접근: {row.get('접근성', '')} / 제약: {row.get('예상 제약', '')} / 연관성: {row.get('정체성 연관성', '')} / 별점: {row.get('적합도(별점)', '')}"])
        csv_data.extend([["최종 선정 건물", ans.get("step2_final_building", "")], ["선정 이유", ans.get("step2_reason", "")]])
        ...
        for i in range(1, 5): csv_data.append([f"컷 {i} 설명", ans.get(f'cut_{i}_desc', '')])
        csv_data.extend([["작품 제목", ans.get("step5_title", "")], ["전시 장소", ans.get("step5_place", "")], ["작품 개요", ans.get("step5_summary", "")], ["지역 정체성 반영", ans.get("step5_identity", "")], ["현장 조건 반영", ans.get("step5_condition", "")], ["남길 변화", ans.get("step5_change", "")]])
...
def generate_portfolio_html(user_key, u_info, view_subj, config, learning_data):
    ...
    for act in config.get("subject_activities", {}).get(view_subj, []):
        ...
        if act in ACTIVITIES: html += generate_html_content(act, ans)
            ...
            for q in config.get("custom_forms", {}).get(act, []): html += f"<h3>{q['label']}</h3><div class='content-box'>{ans.get(q['id'], '')}</div>"
    html += "</body></html>"
    return html
...
def render_activity1_3th(user_key, u_info, current_role):
    ...
    if current_role == "관리자": disabled_flag = False; st.info("💡 교사/관리자 모드입니다. 테스트 저장됩니다.")
...
def render_activity3_3th(user_key, u_info, current_role):
    ...
    edited_s1_df = st.data_editor(s1_df, column_config={"관심도": st.column_config.SelectboxColumn("관심도", options=levels, required=True), "지식수준": st.column_config.SelectboxColumn("지식수준", options=levels, required=True)}, disabled=True if disabled_flag else ["대륙"], hide_index=True, use_container_width=True, key=f"s1_df_{category}")
...
def render_activity1_2nd(user_key, u_info, current_role):
    ...
    step1_2 = st.text_input("2. 이 밈이 대중에게 심어준 우리 지역에 대한 주관적 이미지 (편견 혹은 선입견)", value=ans.get("step1_2", ""), disabled=disabled_flag, key=f"step1_2_{category}")
...
def render_activity3_2nd(user_key, u_info, current_role):
    ...
    default_step1 = [{"구분": "1. 자연·생태", "내가 찾은 정체성 키워드 혹은 문장": "", "근거가 되는 사실·통계·사건": "", "출처(기관명/자료명/연도)": ""}, {"구분": "2. 산업·경제", "내가 찾은 정체성 키워드 혹은 문장": "", "근거가 되는 사실·통계·사건": "", "출처(기관명/자료명/연도)": ""}, {"구분": "3. 역사·문화", "내가 찾은 정체성 키워드 혹은 문장": "", "근거가 되는 사실·통계·사건": "", "출처(기관명/자료명/연도)": ""}]
    ...
    stars = ["⭐", "⭐⭐", "⭐⭐⭐", "⭐⭐⭐⭐", "⭐⭐⭐⭐⭐"]
    default_step2 = [{"건물명": "후보 1: ", "벽면 조건": "", "관람 조건": "", "접근성": "", "예상 제약": "", "정체성 연관성": "", "적합도(별점)": "⭐⭐⭐"}, {"건물명": "후보 2: ", "벽면 조건": "", "관람 조건": "", "접근성": "", "예상 제약": "", "정체성 연관성": "", "적합도(별점)": "⭐⭐⭐"}, {"건물명": "후보 3: ", "벽면 조건": "", "관람 조건": "", "접근성": "", "예상 제약": "", "정체성 연관성": "", "적합도(별점)": "⭐⭐⭐"}]
    step2_df = pd.DataFrame(ans.get("step2_df", default_step2))
    ...
    default_step3 = [{"조건 영역": "1. 물리적 조건 (형태·재질·구조물)", "현장의 실제 조건 (확인한 사실)": "", "작품에 미치는 영향": "", "나의 대응 방안": ""}, {"조건 영역": "2. 빛·환경 조건 (주변 조명·간판·빛공해)", "현장의 실제 조건 (확인한 사실)": "", "작품에 미치는 영향": "", "나의 대응 방안": ""}, {"조건 영역": "3. 시간·계절 조건 (일몰 시각·강수·바람)", "현장의 실제 조건 (확인한 사실)": "", "작품에 미치는 영향": "", "나의 대응 방안": ""}, {"조건 영역": "4. 주민·이웃 조건 (인접 주거·상가·소음)", "현장의 실제 조건 (확인한 사실)": "", "작품에 미치는 영향": "", "나의 대응 방안": ""}, {"조건 영역": "5. 행정·비용 조건 (허가·예산·관리 주체)", "현장의 실제 조건 (확인한 사실)": "", "작품에 미치는 영향": "", "나의 대응 방안": ""}, {"조건 영역": "6. 접근·안전 조건 (동선·차도·배리어프리)", "현장의 실제 조건 (확인한 사실)": "", "작품에 미치는 영향": "", "나의 대응 방안": ""}]
    step3_df = pd.DataFrame(ans.get("step3_df", default_step3))
    ...
    checklist_items = ["Step 1의 정체성 키워드 3개 모두에 출처를 적었다.", "후보 건축물 3곳을 실제로 답사하거나 로드뷰로 확인했다.", "Step 3의 6개 조건 영역을 빈칸 없이 채웠다.", "조건을 문제점으로만 쓰지 않고, 대응 방안까지 모두 적었다.", "스토리보드 4컷이 Step 1의 메시지와 연결되어 있다.", "진로 심화 트랙 산출물을 함께 제출했다.", "작품 설명 카드에 근거 자료를 인용했다."]
    ...
    edited_step6_chk_df = st.data_editor(step6_chk_df, hide_index=True, use_container_width=True, disabled=disabled_flag, key=f"step6_chk_{category}", column_config={"No": st.column_config.NumberColumn("No", width="small"), "확인": st.column_config.CheckboxColumn("확인", default=False)})
    ...
    if not disabled_flag:
        if st.button("저장하기", type="primary", key=f"save_{category}"):
            ...
            new_ans = {
                ...
                "step3_df": edited_step3_df.to_dict('records'),
                "step5_title": step5_title, "step5_place": step5_place, "step5_summary": step5_summary, "step5_identity": step5_identity,
                "step5_condition": step5_condition, "step5_change": step5_change,
                "step6_chk_df": edited_step6_chk_df.to_dict('records'), "step6_ai_df": edited_step6_ai_df.to_dict('records'),
...
def render_class_overview(current_role, u_info, view_subj):
    ...
    if filtered_notices:
        ...
        for notice in filtered_notices:
            ...
            if t or c: st.info(f"**{t}**{badge_text}\n\n{c}")
        st.markdown("---")
    ...
    if current_role == "학생": display_acts = [a for a in acts_for_subj if is_act_visible_for_class(a, user_class_group, app_config)]
...
def inject_custom_scripts():
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        setInterval(() => { fetch(window.parent.location.href, { cache: "no-store" }); }, 180000); 
    });
    </script>
    """, height=0, width=0)
...
if not st.session_state.logged_in:
    ...
    if st.session_state.current_page != "main":
        ...
        if current_role == "학생":
                    ...
                    if acts_for_vis:
                        ...
                        with st.form(f"vis_form_class_{admin_view_subj}"):
                            ...
                            if st.form_submit_button(f"[{admin_view_subj}] 반별 공개/비공개 설정 일괄 저장", type="primary"):
                                ...
                                create_auto_backup(f"[{admin_view_subj}] 수행평가 반별 공개 설정 변경")
                    ...
                    with st.form("add_custom_block"):
                        ...
                        if st.form_submit_button("블록 생성하기", type="primary"):
                            if cb_title and cb_content:
                                ...
                                save_json(CONFIG_FILE, fresh_config)
                                create_auto_backup(f"[{admin_view_subj}] 자유 텍스트 블록 추가")
                ...
                with col_cb2:
                    ...
                    if current_blocks:
                        ...
                        if st.button("선택한 블록 삭제하기", type="primary"):
                            ...
                            save_json(CONFIG_FILE, fresh_config)
                            create_auto_backup(f"[{admin_view_subj}] 자유 텍스트 블록 삭제")
                            st.session_state.admin_save_success = True; st.rerun()
                    ...
                    with st.form("add_dynamic_link"):
                        ...
                        if st.form_submit_button("링크 추가하기", type="primary"):
                            if final_dl_group and new_dl_title and new_dl_url:
                                ...
                                save_json(CONFIG_FILE, fresh_config)
                                create_auto_backup(f"[{admin_view_subj}] 외부 링크 추가")
                                st.session_state.admin_save_success = True; st.rerun()
                ...
                with col_dl2:
                    ...
                    if current_dynamic_links:
                        ...
                        if st.button("선택한 링크 삭제하기", type="primary"):
                            ...
                            save_json(CONFIG_FILE, fresh_config)
                            create_auto_backup(f"[{admin_view_subj}] 외부 링크 삭제")
                            st.session_state.admin_save_success = True; st.rerun()
                ...
                with st.form("upload_mat"):
                    ...
                    if st.form_submit_button("등록", type="primary"):
                        if mat_title and mat_link:
                            ...
                            save_json(CONFIG_FILE, fresh_config)
                            create_auto_backup(f"[{admin_view_subj}] 수업 자료 등록")
                            st.session_state.admin_save_success = True; st.rerun()
                    ...
                    with col_q1:
                        ...
                        if st.button("질문 추가하기", type="primary"):
                            if new_q_label:
                                ...
                                save_json(CONFIG_FILE, fresh_config)
                                create_auto_backup(f"[{edit_act}] 새 질문 추가")
                    ...
                    with col_q2:
                        if cur_form:
                            ...
                            if st.button("선택한 질문 삭제하기", type="primary"):
                                ...
                                save_json(CONFIG_FILE, fresh_config)
                                create_auto_backup(f"[{edit_act}] 질문 삭제")
            ...
            with menu_tabs[2]:
                ...
                if pending_users:
                    ...
                    with col_p1:
                        ...
                        if approve_class_sel != "선택" and st.button(f"✅ [{approve_class_sel}] 대기 학생 일괄 승인", type="primary", use_container_width=True):
                            fresh_users = load_json(USERS_FILE, {})
                            count = 0
                            for uid, u_data in pending_users.items():
                                if u_data.get("class_group") == approve_class_sel and uid in fresh_users:
                                    ...
                                    count += 1
                            save_json(USERS_FILE, fresh_users)
                            create_auto_backup(f"[{approve_class_sel}] 반별 학생 일괄 승인 ({count}명)")
                            st.success(f"✅ {approve_class_sel} 학생 {count}명 승인 완료!"); st.rerun()

                    with col_p2:
                        approve_target = st.selectbox("👤 개별 학생 선택", ["선택"] + list(pending_users.keys()), format_func=lambda x: x if x=="선택" else f"[{pending_users[x].get('subject')}/{pending_users[x].get('class_group')}] {pending_users[x].get('name')} ({pending_users[x].get('id')})")
                        if approve_target != "선택" and st.button("✅ 선택한 학생 승인", type="primary", use_container_width=True):
                            fresh_users = load_json(USERS_FILE, {})
                            if approve_target in fresh_users:
                                fresh_users[approve_target]["approved"] = True
                                save_json(USERS_FILE, fresh_users)
                                create_auto_backup(f"[{pending_users[approve_target].get('name')}] 학생 개별 승인")
                            st.success("승인 완료!"); st.rerun()
                            
                    with col_p3:
                        st.write("")
                        st.write("")
                        if st.button("✅ 대기 중인 모든 학생 일괄 승인", type="primary", use_container_width=True):
                            fresh_users = load_json(USERS_FILE, {})
                ...
                st.markdown("---")
                st.markdown("<h3 style='font-size: 24px; font-weight: 800; margin-top:20px;'>📝 학생 회원 정보(반/이름/학번/비밀번호) 수정</h3>", unsafe_allow_html=True)
                st.info("💡 학생이 가입 시 분반을 잘못 선택했거나 이름/학번/비밀번호 등에 오류가 있는 경우 즉시 수정할 수 있습니다.")
                ...
                if edit_target != "선택":
                    ...
                    with st.form("edit_student_form"):
                        ...
                        with col_e2:
                            ...
                            e_pw = st.text_input("비밀번호 (변경 시에만 입력, 유지 시 기존 값)", value=target_info.get("password", ""))
                        ...
                        if st.form_submit_button("정보 수정 적용", type="primary"):
                            ...
                            if new_key != edit_target:
                                if new_key in fresh_users:
                                    ...
                                    save_json(USERS_FILE, fresh_users)
                                    create_auto_backup(f"[{e_name}] 학생 정보/키값 변경 ({edit_target} ➔ {new_key})")
                                    st.success("✅ 학생 정보가 성공적으로 변경되었습니다."); st.rerun()
                            else:
                                fresh_users[edit_target] = new_info
                                save_json(USERS_FILE, fresh_users)
                                create_auto_backup(f"[{e_name}] 학생 정보 업데이트")
                ...
                with col_ctrl1:
                    ...
                    if del_target != "선택" and st.button("⚠️ 강제 탈퇴(삭제) 실행", type="primary"):
                        fresh_users = load_json(USERS_FILE, {})
                        if del_target in fresh_users: 
                            deleted_name = fresh_users[del_target].get('name', '')
                            del fresh_users[del_target]
                            save_json(USERS_FILE, fresh_users)
                            create_auto_backup(f"[{deleted_name}] 회원 강제 탈퇴")
                ...
                with col_ctrl2:
                    ...
                    new_pw = st.text_input("새로운 비밀번호 입력", key="new_pw_input", type="password")
                    if pw_target != "선택" and st.button("비밀번호 변경 실행", type="primary"):
                        if new_pw:
                            ...
                            if pw_target in fresh_users: 
                                ...
                                save_json(USERS_FILE, fresh_users)
                                create_auto_backup(f"[{fresh_users[pw_target].get('name')}] 비밀번호 강제 변경")
                ...
                if not student_list:
                    ...
                    if view_mode == "👤 특정 학생 실시간 집중 분석":
                        ...
                        if not acts_for_subj:
                            ...
                            selected_view = st.selectbox("다운로드 및 조회할 데이터 범주(수행평가)를 선택하세요", acts_for_subj)
                            st.info("💡 아래 화면에서 각 학생의 실제 작성 내용을 확인하실 수 있으며, 하단 버튼으로 엑셀(CSV) 또는 HTML 형태로 다운로드할 수 있습니다.")
                            st.markdown("---")
                            ...
                            with zipfile.ZipFile(zip_buffer_item, "w", zipfile.ZIP_DEFLATED) as zf_item:
                                for s_uid in student_list:
                                    ...
                                    st.markdown(f"<h3 style='font-size: 22px; font-weight: 800; color: #2c3e50;'>👤 [{u_info_csv.get('subject', '')}] {u_class} - {u_name} ({u_id})</h3>", unsafe_allow_html=True)
                                    ...
                                    for row in act_rows:
                                        q_text, a_text = str(row[0]), str(row[1])
                                        if not q_text and not a_text:
                                            continue
                                        if q_text.startswith("[") and q_text.endswith("]"):
                                            st.markdown(f"<h5 style='color: #0056b3; font-weight: 800; margin-top: 15px;'>{q_text}</h5>", unsafe_allow_html=True)
                                        elif a_text == "":
                                            st.markdown(f"<p style='font-weight: 700; color: #111; margin-bottom: 2px;'>{q_text}</p>", unsafe_allow_html=True)
                                        else:
                                            st.markdown(f"""
                                            <div style='background-color: #f8f9fa; border: 1px solid #e9ecef; border-radius: 6px; padding: 10px 14px; margin-bottom: 10px;'>
                                                <span style='font-weight: 700; color: #333;'>{q_text}</span><br>
                                                <span style='color: #111; font-weight: 500;'>{a_text}</span>
                                            </div>
                                            """, unsafe_allow_html=True)
                                    
                                    single_html = generate_activity_html(selected_view, ans, u_name)
                                    zf_item.writestr(f"{u_class}_{u_name}_{selected_view[:8]}.html", single_html.encode('utf-8-sig'))
                                    has_html_item = True
                            ...
                            st.markdown("<h4 style='font-size: 20px; font-weight: 800;'>📥 일괄 다운로드</h4>", unsafe_allow_html=True)
                            ...
                            with col_down1:
                                if csv_data:
                                    ...
                                    st.download_button(
                                        f"📊 [{selected_view[:8]}] 엑셀(CSV) 세로 양식 다운로드",
                                        data=df_csv.to_csv(index=False, header=False).encode('utf-8-sig'),
                                        file_name=f"{view_subj}_{view_class}_{selected_view[:8]}.csv",
                                        mime='text/csv',
                                        type="primary",
                                        use_container_width=True
                                    )
            ...
            # --- 💾 탭 4: 수동 DB 파일 백업 및 복구 ---
            with menu_tabs[4]:
                st.markdown("<h3 style='font-size: 24px; font-weight: 800; margin-top:20px;'>💾 시스템 데이터베이스(DB) 수동 백업 및 복구</h3>", unsafe_allow_html=True)
                ...
                with col_bk2:
                    ...
                    if st.button("학생 학습 데이터 복구 실행", use_container_width=True):
                        if up_data:
                            ...
                            except Exception: st.error("❌ 올바른 json 파일이 아닙니다.")
            ...
            # --- 🛡️ 탭 5: 자동 백업 센터 (첨부 스크린샷과 완벽히 동일한 UI) ---
            with menu_tabs[5]:
                ...
                total_bk_count = len(bk_files)
                
                total_bytes = 0
                for f in bk_files:
                    try: total_bytes += os.path.getsize(os.path.join(BACKUP_DIR, f))
                    except: pass
                total_mb = round(total_bytes / (1024 * 1024), 1)
                
                # 현재 학습 데이터 보유 학생 수 계산
                ...
                with c_stat2:
                    st.caption("보관 백업")
                    st.markdown(f"<h2 style='color:#34495e; font-size:28px; font-weight:500; margin:0;'>{total_bk_count}개</h2>", unsafe_allow_html=True)
                with c_stat3:
                    st.caption("용량")
