...
def render_activity2_3th(user_key, u_info, current_role):
    ...
    q3_3 = st.text_area("3-3) 만약 그런 장소(공간)이/가 있다면 무엇 때문인가?", value=ans.get("q3_3", ""), disabled=disabled_flag, key=f"q3_3_{category}") # 🌟 Key 에러 수정 유지
...
# =====================================================================
# 🚀 메인 실행부 및 UI 구성 (사이드바 강제 열림 설정 포함)
# =====================================================================
                ...
                if reg_subject and reg_class and reg_id and reg_name and reg_pw:
                    if not agree_privacy: st.error("❌ 개인정보 수집 및 이용에 동의해야 가입할 수 있습니다.")
                        ...
                        if user_key in fresh_users: st.error("❌ 해당 학번이 이미 가입되어 있습니다.")
                            ...
                            save_json(USERS_FILE, fresh_users); create_auto_backup(f"[{reg_name}] 회원가입 신청"); st.success("🎉 가입 완료! 선생님의 승인을 기다려주세요.")
...
if not st.session_state.logged_in:
    ...
    if st.session_state.current_page != "main":
        ...
        if current_role == "학생":
            ...
            menu_tabs = st.tabs(["📌 메인 화면/기한 설정", "🗂️ 수행평가 문항 제작", "👥 회원 관리", "📥 학생 제출 자료 조회 및 관리", "💾 DB 수동 복구", "🛡️ 자동 백업 센터"])
                    ...
                    with st.form("edit_student_form"):
                        ...
                        if btn_delete:
                            ...
                            if edit_target in fresh_data:
                                del fresh_data[edit_target]
                                save_json(DATA_FILE, fresh_data)
                            create_auto_backup(f"[{target_info.get('name')}] 계정 및 데이터 삭제")
            ...
            with menu_tabs[4]:
                ...
                # 🌟 [안전한 JSON 파서] 파일 인코딩(BOM) 에러 완벽 차단 및 스냅샷 자동 분해 기능
