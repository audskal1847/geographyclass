    ...
    if user_key in exceptions:
        try:
            ...
            if get_kst_now() <= exc_dl: 
                ...
                return False, f"🚫 [개별 기한 마감] 연장된 기한({exceptions[user_key]})이 지났습니다."
        except: pass
...
# 🌟 [신규] 특정 학생 예외(연장) 시 비공개 상태여도 화면 강제 노출
...
def init_system():
    with db_lock:
        ...
        if "exceptions" not in current_config: current_config["exceptions"] = {}; needs_update = True # 🌟 개별 권한 저장소 추가
...
def render_activity2_3th(user_key, u_info, current_role):
    ...
    # 🌟 [오류 수정 반영] q3_3 Key 중복 오류 방지 (기존 q3_2 -> q3_3로 교체 완료)
    ...
    if st.session_state.current_page != "main":
        ...
        if current_role == "학생":
            ...
            with menu_tabs[4]:
                ...
                # 🌟 만능 JSON 파서 (인코딩 자동 인식 및 스냅샷 내부 데이터 추출)
                def parse_uploaded_json(uploaded_file, target_key):
                ...
                with col_bk2:
                    ...
                    if st.button("학생 학습 데이터 복구 실행", use_container_width=True):
                        if up_data:
                            parsed_data = parse_uploaded_json(up_data, "learning_data")
                    ...
                    if st.button("회원 정보 복구 실행", use_container_width=True):
                        if up_users:
                            parsed_users = parse_uploaded_json(up_users, "users")
                    ...
                    if st.button("시스템 설정 복구 실행", use_container_width=True):
                        if up_config:
                            parsed_config = parse_uploaded_json(up_config, "config")
